...
p('BLACK_PIECES', 81 * 9)
p('WHITE_PIECES', 81 * 9)
p('BLACK_POCKETS', 18 + 6 * 4, True)
p('WHITE_POCKETS', 18 + 6 * 4, True)
...
pub fn compute_black_pockets_hash(black_pockets: &[u8]) -> u64 {
  black_pockets.iter().enumerate().skip(1).filter(|&(_, &c)| c > 0).fold(0, |acc, (p, &c)| acc ^ get_black_pocket_hash(p as i8, c))
}

pub fn compute_white_pockets_hash(white_pockets: &[u8]) -> u64 {
  white_pockets.iter().enumerate().skip(1).filter(|&(_, &c)| c > 0).fold(0, |acc, (p, &c)| acc ^ get_white_pocket_hash(p as i8, c))
}
